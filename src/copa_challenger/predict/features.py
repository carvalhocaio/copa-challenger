...
def strength_from_stats(
    df: pl.DataFrame, league_avg: float, k: float = SHRINKAGE_K
) -> pl.DataFrame:
    ...
    raw_attack = ((pl.col("goals_for") + pl.col("xg_for")) / 2 / matches) / league_avg
    raw_defense = ((pl.col("goals_against") + pl.col("xg_against")) / 2 / matches) / league_avg
...
def league_average_goals(con: duckdb.DuckDBPyConnection, years: list[int] | None = None) -> float:
    """Média de gols por seleção por jogo (blend gol/xG), no recorte de anos dado."""
    stats = _team_stats(con, years)
    total_adj_goals = (float(stats["goals_for"].sum()) + float(stats["xg_for"].sum())) / 2
    return total_adj_goals / float(stats["matches"].sum())


def team_strength(con: duckdb.DuckDBPyConnection, years: list[int] | None = None) -> pl.DataFrame:
    ...
    stats = _team_stats(con, years)
    avg = league_average_goals(con, years)
    return strength_from_stats(stats, avg)
...
def team_features(
    ...
    strength = team_strength(con, years)
